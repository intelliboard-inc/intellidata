...
class auroramysql_custom_moodle_database_external extends \auroramysql_native_moodle_database {
    ...
    public function set_temptables($db, $temptables) {
        // Open LMS patch INT-21735: re-register the table names on this connection's own temptables
        // object instead of adopting the clone. The clone still references the old (already
        // disposed) connection internally, which makes moodle_temptables::dispose() fatal
        // at shutdown when any temp table is left registered.
        foreach ($temptables->get_temptables() as $tablename) {
            $db->temptables->add_temptable($tablename);
        } // End of patch.
    }
}
